...
추세: ${result.trend}
패턴: ${result.pattern}
지지선: ${result.supportLevels.join(", ")}
저항선: ${result.resistanceLevels.join(", ")}
  ...
  const handleShare = useCallback(async () => {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        ...
        // 사용자가 공유를 취소하면 복사 fallback으로 이어갑니다.
      }
    }
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button onClick={handleDownload} aria-label="분석 결과 JSON 다운로드" className={btnBase}>
            JSON
          </button>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
          <div className="mb-2">
            <Badge color="bg-emerald-500/20 text-emerald-400">LONG</Badge>
          </div>
          <p className="text-sm leading-relaxed text-zinc-300">{result.longView}</p>
        </div>
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
          <div className="mb-2">
            <Badge color="bg-red-500/20 text-red-400">SHORT</Badge>
          </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">신뢰도</p>
            <p className={`mt-0.5 text-xs ${confidenceStyle.text}`}>{confidenceStyle.label}</p>
          </div>
          <span
            className={`text-2xl font-bold tabular-nums ${confidenceStyle.text}`}
            aria-label={`신뢰도 ${result.confidence}퍼센트`}
          >
            {result.confidence}%
          </span>
        </div>
        ...
        >
          <div
            className={`h-full rounded-full transition-all duration-1000 ease-out ${confidenceStyle.bar}`}
            style={{ width: `${result.confidence}%` }}
          />
        </div>
        <div className="mt-1.5 flex justify-between text-xs text-zinc-600" aria-hidden="true">
          <span>0%</span>
          <span>50%</span>
          <span>100%</span>
        </div>
